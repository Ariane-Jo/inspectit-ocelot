...
public class EventRegistryService {
    ...
    private static Map<String, OcelotEventPluginHandler> exporters = new HashMap<>();

    public void sendEventsToExporters(Collection<Event> events) {
        if(events == null) {
            return;
        }
        for(Map.Entry<String, OcelotEventPluginHandler> entry : exporters.entrySet()) {
    ...
    public static void registerHandler(String name, OcelotEventPluginHandler handler) {
        log.info("Registering handler: " + name + " to event exporters");
    ...
    public static void unregisterHandler(String name) {
        log.info("Unregistering handler: " + name + " from event exporters");
